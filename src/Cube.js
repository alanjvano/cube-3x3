...
Cube.prototype.inverse = function(p_str) {
	let per = this.splitPerm(p_str);
	let inverse = Array(per.length);
	console.log(per);
	for (let i = per.length; i > 0; i--) {
		//reverse order
		console.log(per[i]);
		inverse[per.length-i] = clone(per[i-1]);
		//reverse direction
		inverse[per.length-i][1] = !(per[i-1][1]);
	}
...
Cube.prototype.splitPerm = function(p_str) {
	let p = [];
...
Cube.prototype.perm = function(p_str) {
	...
	let p = this.splitPerm(p_str);

	//check for invalid input
	for (let i = 0; i < p.length; i++) {
		if (!valid_perm.includes(p[i][0]) && !valid_rot.includes(p[i][0])) {
	...
	for (let each of p) {
		...
		if (valid_perm.includes(each[0])) {
			this.cube = this.turn(each, this.cube);
		} else {
			this.cube = this.rotate(each);
		}
	}

};

Cube.prototype.rotate = function(dir) {
	//def buffer cycle to adjust
	let cycle = clone(rots[dir[0]]);

	//flip direction if necessary
	if (!dir[1] && !dir[2]) {
		cycle = this.inverse(cycle);
	} else {
		cycle = this.splitPerm(cycle); 
	}

	console.log("rotation cycle", cycle);

	for (let i = 0; i <= dir[2]; i++) {
		for (let each of cycle) {
			this.cube = this.turn(each, this.cube);
		}
	}
	return this.cube;
